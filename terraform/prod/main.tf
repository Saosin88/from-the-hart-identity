    ...
    spec {
      containers {
        ...
        env {
          ...
          value = data.terraform_remote_state.shared.outputs.tech_prod_project_id
        }

        env {
          name  = "AUTH_SERVICE_ACCOUNT_EMAIL"
          value = "auth-firebase-adminsdk-fbsvc@${data.terraform_remote_state.shared.outputs.tech_prod_project_id}.iam.gserviceaccount.com"
        }
...
resource "google_cloud_run_service_iam_member" "auth_service_invoker" {
  ...
  location = google_cloud_run_service.from_the_hart_identity.location
  role     = "roles/run.invoker"
  member   = "serviceAccount:auth-firebase-adminsdk-fbsvc@${data.terraform_remote_state.shared.outputs.tech_prod_project_id}.iam.gserviceaccount.com"
}
